...
export class UserSubscriptionsService {
    ...
    async getAll(userId: number, getUserSubsQuery: GetUserSubsQueryDto): Promise<FormattedSubscription[]> {
        ...
        const options: AssociationGetOptions = {order: [['renewalAt', orderSort]], where: {}};

        if (getUserSubsQuery.limit) options.limit = getUserSubsQuery.limit;

        if (getUserSubsQuery.start || getUserSubsQuery.end) {
            options.where['renewalAt'] = {}

            if (getUserSubsQuery.start) options.where['renewalAt'][Op.gte] = getUserSubsQuery.start
            if (getUserSubsQuery.end) options.where['renewalAt'][Op.lte] = getUserSubsQuery.end
        }
